...
export default function FeatureCircles() {
  ...
  const createCirclePath = (cx: number, cy: number, leftGap: boolean, rightGap: boolean) => {
    let path = '';
    ...
    if (leftGap && rightGap) {
      ...
      path = `M ${start1.x} ${start1.y} A ${circleRadius} ${circleRadius} 0 0 1 ${end1.x} ${end1.y} M ${start2.x} ${start2.y} A ${circleRadius} ${circleRadius} 0 0 1 ${end2.x} ${end2.y}`;
        ...
        path = `M  ${start.x} ${start.y} A ${circleRadius} ${circleRadius} 0 0 1 ${end.x} ${end.y} A ${circleRadius}   ${circleRadius}   0 1 1 ${cx + circleRadius} ${cy}`;
// ... exist existing code 
    } else if (rightGap) {
      ...
      path = `M ${cx + circleRadius} ${cy} A ${circleRadius} ${circleRadius} 0 1 1 ${cx + circleRadius - 0.1} ${cy}`;
    }
        ...
        <g key={index} transform={`translate(${index * 300}, 0)`}> 
            <path 
              ...
              strokeWidth={strokeWidth} 
                    transform={circle.leftGap ? 'rotate(180, 150, 150)' : ''} // Rotate only the border
            />
            <text 
              x="150" 
              y="140" 
